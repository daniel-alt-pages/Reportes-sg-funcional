 ...
 * 
 * Fuente de verdad única: Firestore.
 * Ya no hay fallbacks a archivos JSON estáticos.
 */
...
export async function getAppConfig(): Promise<AppConfig> {
    ...
    if (!snap.exists()) {
        throw new Error('App configuration not found in Firestore (config/app)');
    }
...
export async function getStudentByEmail(email: string): Promise<StudentProfile | null> {
    ...
    const DEBUG_EMAIL_ALIASES: Record<string, string> = {
        'agenteno11sg@gmail.com': 'mendietafonscar.sg.est@gmail.com',
    };
